...
const EmployeeRepository = {
  ...
  async findAll() {
    const { rows } = await db.query('SELECT id, name, email, mobilenumber, empcode, isactive FROM employees ORDER BY id ASC');
  ...
  async create(data) {
    ...
    const { rows } = await db.query(
      'INSERT INTO employees (name, email, mobilenumber, empcode, isactive) VALUES ($1, $2, $3, $4, $5) RETURNING *',
  ...
  async update(id, data) {
    const { name, email, mobilenumber, empcode, isactive } = data;
    const { rows } = await db.query(
      'UPDATE employees SET name=$1, email=$2, mobilenumber=$3, empcode=$4, isactive=$5 WHERE id=$6 RETURNING *',
  ...
  async findByMobileNumber(mobileNumber) {
    const { rows } = await db.query('SELECT * FROM employees WHERE mobilenumber = $1', [mobileNumber]); // Using mobilenumber matching existing schema
    const r = rows[0];
    return r ? new Employee(r.id, r.name, r.email, r.mobilenumber, r.empcode, r.isactive) : null;
  }
};
