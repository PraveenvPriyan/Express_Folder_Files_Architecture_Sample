...
const EmployeeTeleDetailsRepository = {
    async findAll() {
        const { rows } = await db.query('SELECT * FROM employee_tele_details ORDER BY id ASC');
    ...
    async findById(id) {
        const { rows } = await db.query('SELECT * FROM employee_tele_details WHERE id = $1', [id]);
        const r = rows[0];
    ...
    async create(data) {
        ...
        const { rows } = await db.query(
            'INSERT INTO employee_tele_details (empcode, telegramid, mobileno, ismatching, isactive, telegram_username) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
    ...
    async update(id, data) {
        const { empcode, telegramid, mobileno, ismatching, isactive, telegram_username } = data;
        const { rows } = await db.query(
            'UPDATE employee_tele_details SET empcode=$1, telegramid=$2, mobileno=$3, ismatching=$4, isactive=$5, telegram_username=$6 WHERE id=$7 RETURNING *',
    ...
    async delete(id) {
        const result = await db.query('DELETE FROM employee_tele_details WHERE id = $1', [id]);
        return result.rowCount > 0;
    },

    async findByTelegramId(telegramId) {
        const { rows } = await db.query('SELECT * FROM employee_tele_details WHERE telegramid = $1', [telegramId]);
        const r = rows[0];
        return r ? new Employee_tele_details(r.id, r.empcode, r.telegramid, r.mobileno, r.ismatching, r.isactive, r.telegram_username) : null;
    }
};
